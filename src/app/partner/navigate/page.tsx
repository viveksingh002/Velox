...
function OtpSection({ bookingId, onVerified }: { bookingId: string; onVerified: () => void }) {
  ...
  const verify = async () => {
    ...
    setVerifying(true);
    try {
      const res  = await fetch(`${API}/booking/${bookingId}/verify-otp`, {
        method: "POST",
      ...
      <div style={{ padding: "1.75rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: "1.25rem" }}>
          ...
          <div style={{ background: "#f9fafb", borderRadius: 12, padding: "12px 14px", border: "1px solid #f3f4f6" }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 800, color: "#0a0a0a", textTransform: "capitalize" }}>{booking.model || booking.vehicle || "—"}</div>
          </div>
        </div>
...
useEffect(() => {
  ...
  const handleArrive = async () => {
    ...
    setArriving(true);
    try {
      const res  = await fetch(`${API}/booking/${booking._id}/arrive`, { method: "PATCH" });
      const data = await res.json();
      if (data.success) {
  ...
  const handleComplete = async () => {
    ...
    setCompleting(true);
    try {
      const res  = await fetch(`${API}/booking/${booking._id}/complete`, { method: "PATCH" });
      const data = await res.json();
      if (data.success) {
        setCompleting(false);
        setShowRating(true);
  ...
  if (loading) {
    return (
      <div style={{ position: "fixed", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", background: "#f5f4f0", fontFamily: "'Inter',system-ui,sans-serif" }}>
        <div style={{ width: 34, height: 34, border: "3px solid #e6e6e3", borderTopColor: "#0a0a0a", borderRadius: "50%", animation: "spin 1s linear infinite" }} />
  ...
  if (!booking) {
    return (
      <div style={{ position: "fixed", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "#f5f4f0", fontFamily: "'Inter',system-ui,sans-serif", gap: 16 }}>
        <p style={{ fontSize: 16, fontWeight: 600, color: "#6b6b66" }}>No active ride found.</p>
        <a href="/partner/pending-requests" style={{ padding: "11px 24px", background: "#0a0a0a", color: "#fff", borderRadius: 12, fontSize: 14, fontWeight: 600, textDecoration: "none" }}>View Pending Requests</a>
      </div>
    );
  }
  ...
  return (
    <div style={{ position: "fixed", inset: 0, fontFamily: "'Inter',system-ui,sans-serif", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <style>{`
      ...
      <div style={{ height: sheetH, flexShrink: 0, background: "#fff", borderTopLeftRadius: 24, borderTopRightRadius: 24, boxShadow: "0 -20px 60px rgba(0,0,0,0.18)", display: "flex", flexDirection: "column", transition: "height 0.35s cubic-bezier(0.4,0,0.2,1)", position: "relative" }}>
        ...
        <div style={{ padding: "6px 20px 100px", overflowY: "auto", flex: 1 }}>
          ...
          {booking.status === "arrived" && (
            <OtpSection bookingId={booking._id} onVerified={onOtpVerified} />
          )}
