...
type EarningDay = { day: string; amount: number };

const DEFAULT_EARNINGS: EarningDay[] = [
  { day: "Mon", amount: 0 },
  { day: "Tue", amount: 0 },
  { day: "Wed", amount: 0 },
  { day: "Thu", amount: 0 },
    ...
    <>
      <div style={{ background: "#111827", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 60, position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <div className="nav-links" style={{ display: "flex", gap: 28 }}>
          {NAV_LINKS.map((l) => (
            <a key={l.label} href={l.href} style={{ fontSize: 13.5, color: "rgba(255,255,255,0.65)", textDecoration: "none", fontWeight: 500 }}
...
function PricingModal({ onClose, onSubmitted }: { onClose: () => void; onSubmitted: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      else setError(data.message || "Failed to submit pricing");
    } catch { setError("Failed to submit pricing. Please try again."); }
      ...
      <div onClick={(e) => e.stopPropagation()} className="pricing-modal-scroll" style={{ width: "100%", maxWidth: 440, background: "#fff", borderRadius: 24, boxShadow: "0 4px 40px rgba(0,0,0,0.25)", maxHeight: "90vh", overflowY: "auto" }}>
        <div style={{ padding: "28px 28px 32px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
            ...
            <button onClick={onClose} style={{ width: 32, height: 32, borderRadius: "50%", border: "1.5px solid #e5e7eb", background: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <label htmlFor="vehicleImageInput" style={{ display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 8, width: "100%", height: 160, borderRadius: 16, cursor: "pointer", marginBottom: 20, border: vehicleImage ? "1.5px solid #e5e7eb" : "2px dashed #d1d5db", background: vehicleImage ? "transparent" : "#fafafa", overflow: "hidden" }}>
            {vehicleImage ? <img src={vehicleImage} alt="Vehicle" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : (
              <>
          ...
          {[
            { label: "Base fare (₹)",          val: baseFare,      set: setBaseFare,      ph: "40" },
            { label: "Price per KM (₹)",        val: pricePerKm,    set: setPricePerKm,    ph: "10" },
            { label: "Waiting charge / min (₹)", val: waitingCharge, set: setWaitingCharge, ph: "0"  },
          ].map((f) => (
...
function StatusBanner({ status, videoKycStatus, videoKycResult, pricingStatus, pricingRejectReason, videoKycRoomId, onRetry, onSetPricing, showLiveBanner, router }: {
  status: VendorStatus; videoKycStatus: VideoKycStatus; videoKycResult: VideoKycResult;
  pricingStatus: PricingStatus; pricingRejectReason: string; videoKycRoomId: string | null;
  onRetry: () => void; onSetPricing: () => void; showLiveBanner: boolean; router: ReturnType<typeof useRouter>;
}) {
  const handleJoinCall = () => { if (videoKycRoomId) window.location.href = `/video-kyc/${videoKycRoomId}?role=vendor`; };
    ...
    <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e5e7eb", padding: "20px 24px", marginBottom: 20 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button onClick={onRetry} style={{ padding: "10px 18px", background: "#111827", color: "#fff", borderRadius: 10, fontWeight: 600, fontSize: 13, border: "none", cursor: "pointer" }}>Re-apply →</button>
          <a href="mailto:support@velox.in" style={{ padding: "10px 18px", background: "#fff", color: "#374151", borderRadius: 10, fontWeight: 600, fontSize: 13, border: "1.5px solid #e5e7eb", textDecoration: "none" }}>Contact Support</a>
        </div>
      </div>
    </div>
  );
  ...
  if (videoKycStatus !== "completed") return (
    <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e5e7eb", padding: "20px 24px", marginBottom: 20 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
        ...
        <div style={{ flex: 1, minWidth: 160 }}>
          ...
          <p style={{ fontSize: 13, color: "#6b7280" }}>{videoKycStatus === "in_progress" ? "Admin has started your Video KYC call. Join now." : "Your documents are verified. Wait for admin to start your Video KYC."}</p>
        </div>
        {videoKycStatus === "in_progress"
          ? <button onClick={handleJoinCall} style={{ padding: "10px 20px", background: "#16a34a", color: "#fff", borderRadius: 10, fontWeight: 600, fontSize: 13, border: "none", cursor: "pointer" }}>Join Call →</button>
          : <div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 8, height: 8, borderRadius: "50%", background: "#f59e0b", animation: "pulse 1.5s infinite" }} /><span style={{ fontSize: 12, color: "#6b7280" }}>Waiting for admin…</span></div>
        }
      </div>
      <style>{`@keyframes pulse{0%,100%{opacity:1}50%{opacity:.3}}`}</style>
    </div>
  ...
  if (videoKycResult === "rejected") return (
    <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e5e7eb", padding: "20px 24px", marginBottom: 20 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
        <div style={{ width: 44, height: 44, borderRadius: 12, background: "#dc2626", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 20 }}>❌</div>
        <div style={{ flex: 1, minWidth: 160 }}>
  ...
  if (!pricingStatus || pricingStatus === "pending") return (
    <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e5e7eb", padding: "20px 24px", marginBottom: 20 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
        ...
        <button onClick={onSetPricing} style={{ padding: "10px 20px", background: "#2563eb", color: "#fff", borderRadius: 10, fontWeight: 600, fontSize: 13, border: "none", cursor: "pointer" }}>Set Pricing →</button>
      </div>
    </div>
  );

  if (pricingStatus === "submitted") return (
    <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e5e7eb", padding: "20px 24px", marginBottom: 20 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
        <div style={{ width: 44, height: 44, borderRadius: 12, background: "#111827", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 20 }}>⏳</div>
        <div style={{ flex: 1, minWidth: 160 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 8, height: 8, borderRadius: "50%", background: "#22c55e", animation: "pulse 1.5s infinite" }} /><span style={{ fontSize: 12, color: "#6b7280" }}>Checking…</span></div>
      </div>
      <style>{`@keyframes pulse{0%,100%{opacity:1}50%{opacity:.3}}`}</style>
    </div>
  );

  if (pricingStatus === "rejected") return (
    <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e5e7eb", padding: "20px 24px", marginBottom: 20 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
        <div style={{ width: 44, height: 44, borderRadius: 12, background: "#dc2626", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 20 }}>❌</div>
        <div style={{ flex: 1, minWidth: 160 }}>
        ...
        <button onClick={onSetPricing} style={{ padding: "10px 20px", background: "#111827", color: "#fff", borderRadius: 10, fontWeight: 600, fontSize: 13, border: "none", cursor: "pointer" }}>Resubmit →</button>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ background: "#111827", borderRadius: 16, padding: "24px", marginBottom: 20 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }}>
        <p style={{ fontSize: 17, fontWeight: 700, color: "#fff" }}>🚀 You're Live</p>
        <button onClick={() => router.push("/partner/active-ride")} style={{ padding: "10px 20px", background: "#fff", color: "#111827", borderRadius: 10, fontWeight: 600, fontSize: 13, border: "none", cursor: "pointer" }}>
          Go to Orders →
        </button>
      </div>
    </div>
...
function EarningsChart({ data }: { data: EarningDay[] }) {
        ...
        {total === 0 ? (
          <div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {data.map((d) => <p key={d.day} style={{ flex: 1, textAlign: "center", fontSize: 11, color: "#d1d5db", fontWeight: 500 }}>{d.day}</p>)}
            </div>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {data.map((d) => <p key={d.day} style={{ flex: 1, textAlign: "center", fontSize: 11, color: "#9ca3af", fontWeight: 500 }}>{d.day}</p>)}
            </div>
          </div>
        )}
      </div>
...
export default function PartnerDashboard() {
  ...
  const [earningsData,       setEarningsData]       = useState<EarningDay[]>(DEFAULT_EARNINGS);
...
const fetchEarnings = async () => {
  ...
  useEffect(() => {
    ...
    fetchStatus();
    fetchEarnings();
    const interval = setInterval(() => { fetchStatus(); fetchEarnings(); }, 10000);
    return () => clearInterval(interval);
  }, []);

  useEffect(() => {
    if (pricingStatus === "approved") {
      const t = setTimeout(() => setShowLiveBanner(true), 1000);
      return () => clearTimeout(t);
    } else { setShowLiveBanner(false); }
  }, [pricingStatus]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f3f4f6", fontFamily: "Inter,sans-serif" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 20px 48px" }}>
        ...
        <StatusBanner status={status} videoKycStatus={videoKycStatus} videoKycResult={videoKycResult} pricingStatus={pricingStatus} pricingRejectReason={pricingRejectReason} videoKycRoomId={videoKycRoomId} onRetry={handleRetry} onSetPricing={() => setShowPricingModal(true)} showLiveBanner={showLiveBanner} router={router} />
        <EarningsChart data={earningsData} />
      </div>
      {showPricingModal && <PricingModal onClose={() => setShowPricingModal(false)} onSubmitted={fetchStatus} />}
    </div>
  );
}