...
// ── Mock Data ───────────────────────────────────────────────────────────────
const MOCK_REQUESTS = [
  { id: "REQ-3012", customer: "Arjun Singh",   rating: 4.8, trips: 34, pickup: "Sector 18, Noida",            drop: "Connaught Place, Delhi",       distance: "18.3 km", fare: "₹340", eta: "4 min",  vehicleNeeded: "car",     expiresIn: 28 },
  { id: "REQ-3011", customer: "Fatima Syed",   rating: 4.5, trips: 12, pickup: "Lajpat Nagar Metro Station",  drop: "Greater Kailash Part 1",       distance: "3.8 km",  fare: "₹98",  eta: "2 min",  vehicleNeeded: "auto",    expiresIn: 18 },
  { id: "REQ-3010", customer: "Rohit Sharma",  rating: 4.9, trips: 89, pickup: "Dwarka Sector 10",            drop: "IGI Airport Terminal 3",       distance: "11.2 km", fare: "₹220", eta: "6 min",  vehicleNeeded: "car",     expiresIn: 42 },
  { id: "REQ-3009", customer: "Kavya Reddy",   rating: 4.3, trips:  7, pickup: "Anna Nagar, Chennai",         drop: "T Nagar Bus Terminus",         distance: "5.6 km",  fare: "₹115", eta: "3 min",  vehicleNeeded: "bike",    expiresIn: 11 },
  { id: "REQ-3008", customer: "Manish Tiwari", rating: 4.6, trips: 51, pickup: "Banjara Hills Road No. 12",   drop: "Jubilee Hills Check Post",     distance: "2.4 km",  fare: "₹72",  eta: "5 min",  vehicleNeeded: "auto",    expiresIn: 33 },
];

type Request = typeof MOCK_REQUESTS[0];
...
function RequestCard({ req, onAccept, onDecline }: { req: Request; onAccept: (id: string) => void; onDecline: (id: string) => void; }) {
  ...
  const vehicleIcon: Record<string, JSX.Element> = {
      ...
      <div style={{ padding: "18px 20px" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          {[
            ...
            { label: req.vehicleNeeded, icon: vehicleIcon[req.vehicleNeeded] || null },
          ].map((chip) => (
        ...
        {!expired && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 10 }}>
            <button onClick={() => onDecline(req.id)} style={{ padding: "12px", borderRadius: 12, border: "1.5px solid #e5e7eb", background: "#fff", color: "#6b7280", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
              Decline
            </button>
            <button onClick={() => onAccept(req.id)} style={{ padding: "12px", borderRadius: 12, border: "none", background: "#111827", color: "#fff", fontSize: 14, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
              Accept Ride
...
export default function PendingRequestsPage() {
  ...
  const handleAccept = (id: string) => {
    // TODO: POST /api/vendor/accept-ride/:id
    setRequests((prev) => prev.filter((r) => r.id !== id));
    showToast("✓ Ride accepted! Check Active Ride.", "accept");
  };

  const handleDecline = (id: string) => {
    // TODO: POST /api/vendor/decline-ride/:id
    setRequests((prev) => prev.filter((r) => r.id !== id));
    ...
    <div style={{ minHeight: "100vh", background: "#f3f4f6", fontFamily: "Inter,sans-serif" }}>
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "0 24px 48px" }}>
        ...
        <div style={{ marginBottom: 24, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          {requests.length > 0 && (
            <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#111827", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 13, fontWeight: 800 }}>
              {requests.length}
            </div>
          )}
        </div>
