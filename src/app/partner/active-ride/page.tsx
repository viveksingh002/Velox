    ...
    const init = async () => {
      try {
        ...
        const carIcon = L.divIcon({
          html: `<div style="width:56px;height:56px;border-radius:50%;background:#0a0a0a;display:flex;align-items:center;justify-content:center;box-shadow:0 12px 28px rgba(0,0,0,0.35);border:3px solid #fff;"><svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 16H9m10 0h3v-3.15a1 1 0 0 0-.84-.99L16 11l-2.7-3.6a1 1 0 0 0-.8-.4H5.24a2 2 0 0 0-1.8 1.1l-.8 1.63A6 6 0 0 0 2 12.42V16h2"/><circle cx="6.5" cy="16.5" r="2.5"/><circle cx="16.5" cy="16.5" r="2.5"/></svg></div>`,
...
function OtpSection({ expectedOtp, bookingId, onVerified }: { expectedOtp: string; bookingId: string; onVerified: () => void }) {
...
export default function ActiveRidePage() {
  ...
  const handleArrive = async () => {
    setArriving(true);
    try {
      await fetch(`${API}/booking/${booking._id}/arrive`, { method: "PATCH" });
    } catch {}
  ...
  const handleComplete = async () => {
    setCompleting(true);
    try {
      await fetch(`${API}/booking/${booking._id}/complete`, { method: "PATCH" });
    } catch {}
    setCompleting(false);
    setShowRating(true);
  };
      ...
      <div style={{ height: sheetH, flexShrink: 0, background: "#fff", borderTopLeftRadius: 24, borderTopRightRadius: 24, boxShadow: "0 -20px 60px rgba(0,0,0,0.18)", display: "flex", flexDirection: "column", transition: "height 0.35s cubic-bezier(0.4,0,0.2,1)", position: "relative" }}>
        ...
        <div style={{ padding: "6px 20px 100px", overflowY: "auto", flex: 1 }}>
          ...
          {booking.status === "arrived" && (
            <OtpSection expectedOtp={booking.otp} bookingId={booking._id} onVerified={onOtpVerified} />
          )}
